...
def CreateFilterMatrix(Nt,to,tf):
    ...
    fbc1 = 0.30e6#1.25e6 # cutoff frequency [Hz]
    fbc2 = 1.25e6 # 3.8e6 cutoff frequency [Hz]
...
##############################################################################
def applyfilter(to,tf,Nt,Ns,A):
    
    print('Applying filter to j-wave sinogram...');
    FM = CreateFilterMatrix(Nt,to,tf)
    An = np.kron(np.eye(Ns,dtype='float32'),FM)@A
...
###############################################################################
def createJForwMatdotdet(Ns,Nt,dx,nx,dsa,vs,to,tf,nhie,LBW,AP,saveA): # 
