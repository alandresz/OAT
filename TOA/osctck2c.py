...
class Osctck(object):
    ...
    def __init__(self, resource:str):
        ...
        self._channels = (1,2,)
        self._triggerSource = 'EXT' #source can be CH1, CH2, EXT, EXT5, LINE
        self._triggerLevel = 0  #coupling can be AC or DC
        self._triggerSlope = 'RISE' #slope can be FALL or RISe
        self._triggerMode = 'NORM' #mode can be AUTO or NORMal
        self._triggerCoup = 'AC'
        self._acquisition = 1
        self._vAutoScale = False
        self._chanband = ('OFF','OFF')
        self._chaninv = ('OFF','OFF')
        self._chancoup = ('AC','AC') #coupling can be AC, DC or GND
    ...
    def __call__(self):
        ...
        for ch in range (len(self._channels)):
            self._osci.write("CH"+str(self._channels[ch])+":BANDWIDTH " + self._chanband[ch])
            self._osci.write("CH"+str(self._channels[ch])+":INV " + self._chaninv[ch]) 
            self._osci.write("CH"+str(self._channels[ch])+":COUP " + self._chancoup[ch]) 
        ...
        if self._vAutoScale == True:
            asiter = 2
            for ii in range(asiter):
                self.setAcquisition(1)
                
                vScale1 = self.getVertScale(self._channels[0])
                maxValue1 = np.max(np.absolute(self.getVertValues(self._channels[0])))
                vScale1 = maxValue1/7
                
                vScale2 = self.getVertScale(self._channels[1])
                maxValue2 = np.max(np.absolute(self.getVertValues(self._channels[1])))
                vScale2 = maxValue2/3.5
                
                self.setVertScale(self._channels[0],vScale1)
                self.setVertScale(self._channels[1],vScale2)
    ...
    def config(self, channels:tuple, triggerSource:str, triggerLevel:float, triggerSlope:str, triggerMode:str, triggerCoup:str, acquisition:int, vAutoScale:bool, chanband:str, chaninv:str):
    ...
    def setAvgAcquisition(self, nAvg):
        ...
        time.sleep(2*float(nAvg)/self.getTriggerFreq()) 
    ...
    def useAlternativeAutorange(self, channel):        
        self.run()
        self.setAcquisition(1)
        self._osci.write("AUTORANGE:SETTINGS VERTICAL")
        self._osci.write("AUTOR:STATE ON")
        time.sleep(2)
        self._osci.write("AUTOR:STATE OFF")
